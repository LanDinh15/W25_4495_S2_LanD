...
# Filter the data based on type, Rating, and Year
...
# Plot the country-wise duration bar chart
with col1:
    st.subheader("Countries by Duration")
    fig = px.bar(top_10_countries, x="country", y="duration", text=['${:,.2f}'.format(x) for x in top_10_countries["duration"]],
                  color="duration",  # Color based on duration values
                    color_continuous_scale="Reds",  # Apply red gradient
                 template="seaborn")
